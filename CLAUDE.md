...
# Slack ingest — slackdump export → LinkMind backfill (Phase C wave-2, 2026-05-19~)
# ⚠️ GPU OOM 방지를 위해 ingest 도중에는 backend uvicorn 종료 권장 (--stop)
bash scripts/step5_run_dev.sh --stop                               # 임베딩 모델 GPU 차지 해제
...
### Phase C wave-2 (2026-05-19 늦은 저녁, 모듈 완료 / ingest 실행 중) — Slack 워크스페이스 일회성 backfill

사용자가 Slack 구독 해제 직전이라 시한 리스크. raw-first (§2) 라 wiki 모델
무관하게 안전, 오히려 wiki 설계 시 실 데이터 (thread + 첨부 + 다양 채널) 로
검증 가능. **Telegram 패턴 일관 — `backend/ingest/slack/` 신규**.

**Slack export (slackdump standard)**:
- ✅ `bash scripts/slack_export.sh` 로 새 export — **183 채널 / 14241 메시지 /
  625 MB / 16 첨부**, `archive/slack_export/full_2026-05-19_20-04-58/` + `latest`
  symlink. token/cookie cache 살아있어 재인증 불필요.

**Backend (`backend/ingest/slack/` 신규, Telegram 패턴 미러)**:
- ✅ **`export_parser.py`** — slackdump standard 파싱 (channel 디렉토리 + 날짜별
  JSON + `attachments/`). `SlackMessage` / `SlackAttachment` dataclass. 핵심:
  - **mrkdwn entity 정리** (`clean_mrkdwn_text`): `<url|label>` → label,
    `<@U123>` → 제거, `<#C|name>` → `#name`, `<!here>` → `@here`, HTML entity 디코딩
  - **URL 추출** (`extract_urls`): blocks의 link element 우선 → mrkdwn `<url|label>`
    → raw URL. dedup + trailing 구두점 strip
  - **시스템 메시지 skip** (`_SKIP_SUBTYPES`): channel_join/leave/topic/purpose/
    bot_message/reminder_add 등 raw 가치 낮은 것
  - **thread 그룹핑 보존**: 채널 단위 ts 정렬 → pass 1 으로 thread parent (ts==thread_ts)
    의 cleaned text 모음 → pass 2 yield 시 자식의 `parent_text` 에 자동 전파.
    사용자가 Slack 에서 수동으로 만들어둔 multi-modality 묶음 (논문 제목 = 부모 /
    arxiv·github·youtube URL = 자식) 을 caption 으로 보존
  - **첨부 매핑** (`extract_attachments`): `attachments/<file_id>-<name>` 로 resolve,
    파일 없으면 skip (slackdump 실패한 케이스)
  - **channels.json / users.json 메타** 로 channel_id / user display_name 매핑
  - **permalink** 생성 (`{workspace_url}/archives/{channel_id}/p{ts.replace(".","")}`)
- ✅ **`__init__.py`** — `ingest_slack_message` (단일) + `ingest_slack_export` (폴더):
  - URL 자동 host 라우팅 — `_classify_url` → ingest_url/youtube/github/pdf 분기
    (Telegram 흐름 재사용)
  - 첨부 → `ingest_document` (PDF/DOCX/PPTX/TXT/MD 통합)
  - **caption 정책**: thread 자식이면 `parent_text` / 첨부면 본문 / URL 만이면
    `_strip_urls_for_caption` (Telegram helper 재사용). thread 부모 메모도 caption
  - URL/첨부 없고 cleaned text >= 20자면 `source_type='slack'` note 저장
  - `source_metadata['slack']` 에 ts/channel/channel_id/user/thread_ts/permalink/
    is_thread_parent 보존 (note + 첨부 케이스. URL 분기는 provenance 한계 — 다음 wave)
- ✅ **`__main__.py`** — CLI (`python -m backend.ingest.slack <export> [옵션]`):
  - `--channel <name>`, `--workspace-url`, `--force`, `--no-progress`, `--issues-path`,
    `--no-issues`
  - **tqdm 진행률**: 첫 패스로 메시지 카운트 → bar + postfix (현재 채널 / 누적
    urls / errs / iss). `mininterval=0.5` 로 stderr 폭주 방지
  - **이슈 manifest 자동 보존** — `_detect_url_issue` 가 "exception" / "placeholder"
    분류. 기본 path `<export_dir 부모>/issues/<timestamp>/manifest.json` (사용자
    정책 2026-05-19: archive/slack_export/ 하위에만 저장, /tmp 휘발성 금지)

**검증 (실 데이터)**:
- ✅ **단일 채널** `robot-action-foundation` (1 URL: gtsam.org) — chunks=10 +
  한국어 summary 1003자 + tags 6개. **end-to-end 작동 확인**.
- ✅ **thread 채널** `가-공부-논문쓰기-image-composition-이미지-물체-추가` (248 메시지)
  — 218 URLs + 55 notes ingest, thread parent_text → 자식 caption 으로 잘 전파 확인.
  github=63 · pdf=12 · url=107 · slack note=53 · youtube=5, avg summary 600-800자.

**부수 fix (Slack ingest 도중 발견)**:
- ✅ **`_classify_url` 의 `/pdf/` path 인식** (`backend/api/ingest.py`):
  - 기존엔 `.pdf` 확장자만 검사 → `arxiv.org/pdf/2106.14490` 같은 URL 이 url 분기로
    잘못 라우팅돼서 readability fallback 만 도는 placeholder item 만 생기던 버그
  - fix: `parsed.path` 에 `/pdf/` 세그먼트 있어도 pdf 분기 (arxiv / openaccess.thecvf
    / openreview 등 일반 패턴). 회귀 테스트 추가 (`test_classify_pdf_path_segment`).
  - thread 채널 검증에서 발견된 9개 placeholder (8 arxiv pdf URL + 1 unite.ai)
    DELETE 후 사용자가 전체 ingest 시 PDF 흐름으로 재라우팅됨
- ✅ **GPU OOM 해결** — vLLM (qwen2.5-7B, 18 GB) + backend uvicorn 의 bge-m3 (3.78 GB)
  가 GPU 거의 점유 → CLI ingest 의 추가 bge-m3 가 OOM. 단기 fix: `bash scripts/
  step5_run_dev.sh --stop` 으로 ingest 중에만 uvicorn 끄기. **장기 fix 후보 wave-5+**:
  bge-m3 를 **TEI (Text Embedding Inference) 컨테이너** 로 분리 → 모든 프로세스가
  같은 HTTP 서버 호출 → 모델 1번만 GPU 에. CLAUDE.md §12 Phase 2 backlog 의 항목.

**스크립트**:
- ✅ **`scripts/slack_ingest_all.sh`** — 한 줄로 전체 backfill:
  - **사전 점검** — backend uvicorn 가동 중이면 OOM 경고 + 확인 prompt, GPU 여유
    < 1500 MiB 경고, vLLM 응답 점검
  - workspace_url 기본값 `https://w1710672365-sjj477000.slack.com` (channels.json
    에서 본 실제 team URL)
  - log `/tmp/linkmind-slack-ingest-<ts>.log` 자동, issues manifest 는 CLI 가
    archive 하위에 자동 저장
  - 중단 안전 (Ctrl+C OK — hash dedup 으로 재실행 시 skip)

**테스트** (`tests/test_slack_parser.py` 신규):
- 46 케이스 (`pytest -m cpu` 에 포함) — mrkdwn 정리 (9), URL 추출 (6), 첨부 (3),
  helper (5), 메타 로더 (4), parse_slack_export (11), `_resolve_caption` 분기 (5),
  fixture (3). `tests/resources/slack_export_sample/` (channels.json + users.json +
  test-channel/2026-05-19.json + attachments/) 신규.
- `_classify_url` 회귀 테스트 1 케이스 추가 (`/pdf/` path).
- 총 58 tests (slack 46 + url_classify 12) 통과.
...
| 순서 | 항목 | 규모 | 상태 |
|---|---|---|---|
| **0a** | **C wave-2 — Slack 일회성 backfill (모듈)** | ~6시간 | ✅ 완료 (2026-05-19 늦은 저녁). 모듈/CLI/테스트/스크립트/검증 모두. |
| **0b** | **C wave-2 — 전체 14241 메시지 ingest** | ~2일 | 🚧 진행 중 (tmux background). 완료되면 archive/slack_export/issues/<ts>/manifest.json 생성됨. |
| **0c** | **C wave-2 — issues manifest 분석** (ingest 끝난 후 첫 작업) | 작음 | `archive/slack_export/issues/<ts>/manifest.json` 의 placeholder / exception URL 들 — 도메인별 카운트, issue 유형별 분류 (LinkedIn login wall / mp4 video / 정적 project page / 단축 URL / arxiv abs 변환 가능 등). 어떤 패턴이 몇 % 인지 파악 후 다음 단계 결정. |
| **0d** | **C wave-2 — issues 패턴별 재처리 로직** (manifest 분석 결과 기반) | 중 | 각 패턴 별 fix: arxiv `/pdf/`→`/abs/` 변환 retry / archive.org Wayback Machine fallback / t.ly 같은 단축 URL 펼치기 / mp4·image 만 있는 메시지는 첨부 attachment 로 라우팅 / LinkedIn login wall 은 skip 표시. 새 모듈 `backend/ingest/issue_reprocess.py` 같은 형태로. 사용자 자료 손실 최소화가 목적. |
| **1** | **D10 — llm_wiki 아키텍처 도입** | 큼 (여러 세션) | issues 정리 끝난 후. karpathy 의 llm_wiki + vlm_wiki + multi-agent + 자가학습. 일반 RAG 대신 **topic = wiki 페이지** 단위. **이게 LinkMind 의 진짜 정체성** ([[project-llm-wiki-arch]] memory). 첫 세션은 plan 단계 — `external/karpathy/llm_wiki/` 분석 + LinkMind 의 items/topics/categories 매핑 + `backend/agents/` 디렉토리 설계 + multi-agent 흐름 (retriever / writer / critic / fact-checker / linker) + `/wiki/{slug}` endpoint 윤곽. |
...
| **인프라 후보 (wave-5+)** | **TEI 임베딩 분리** | 반나절 | bge-m3 를 단독 컨테이너 (Text Embedding Inference) 로 → 모든 프로세스 (backend uvicorn / CLI ingest / watcher) 가 같은 HTTP 서버 호출 → GPU 에 모델 1번만 → OOM 영원히 해결. Slack ingest 도중 발견된 임시 fix (uvicorn stop) 의 근본 해결. CLAUDE.md §12 Phase 2 backlog 의 항목. |

**Slack backfill 완료 항목** (2026-05-19 늦은 저녁, ✅):
1. ✅ `bash scripts/slack_export.sh` — 183 채널 / 14241 메시지 / 625 MB / 16 첨부 export
2. ✅ `backend/ingest/slack/{export_parser,__init__,__main__}.py` 신규 (Telegram 패턴 미러)
3. ✅ CLI: `python -m backend.ingest.slack` + `bash scripts/slack_ingest_all.sh` (사전 점검 + tqdm + archive 하위 issues manifest)
4. ✅ 단위 테스트 `tests/test_slack_parser.py` 46 케이스 + fixture
5. ✅ 검증: 단일 채널 (chunks=10) + thread 채널 248 메시지 (218 URLs + 55 notes)
6. ✅ 부수 fix: `_classify_url` 의 `/pdf/` path 인식 (arxiv pdf URL 들 정상 라우팅) + GPU OOM 회피 (uvicorn 잠시 stop)
7. 🚧 **진행 중**: 전체 워크스페이스 ingest (`bash scripts/slack_ingest_all.sh`, tmux). 평균 ~12 s/msg, ETA ~2일.
